...
from ibm_watson_machine_learning.tests.utils import save_data_to_container
...
class FeatureSteps:
    ...
    def write_data_to_container(self):
        print(self.data_storage.api_client.project_id)
    ...
    def data_reference_setup_c(self):
        ...
        assert len(self.data_storage.train_data_connections) == 1, "train_data_connection length it is different than '1'"
    ...
    def deployment_creation_in_project(self):
        if not self.data_storage.SPACE_ONLY:
            self.data_storage.api_client.set.default_project(self.data_storage.project_id)
        meta_props = {
            self.data_storage.api_client.deployments.ConfigurationMetaNames.NAME: "PT deployment SDK tests",
            self.data_storage.api_client.deployments.ConfigurationMetaNames.ONLINE: {}
        }
        deployments_details = self.data_storage.api_client.deployments.create(self.data_storage.stored_model_id,
                                                                              meta_props=meta_props)
        self.data_storage.deployment_id = self.data_storage.api_client.deployments.get_id(deployments_details)
        assert self.data_storage.deployment_id is not None, "deployment_id cannot be None"

        assert deployments_details['entity']['status']['state'] == 'ready', f"Deployment state is: {deployments_details['entity']['status']['state']} and should be 'ready'"
        print(deployments_details)

    def deployment_creation_with_prompted_asset(self):
        meta_props = {
            self.data_storage.api_client.deployments.ConfigurationMetaNames.NAME: "PT deployment SDK tests",
            self.data_storage.api_client.deployments.ConfigurationMetaNames.ONLINE: {}
        }
        deployments_details = self.data_storage.api_client.deployments.create(self.data_storage.promoted_model_id,
                                                                              meta_props=meta_props)
        self.data_storage.deployment_id = self.data_storage.api_client.deployments.get_id(deployments_details)
    ...
    def delete_deployments(self):
        if not self.data_storage.SPACE_ONLY:
            self.data_storage.api_client.set.default_project(self.data_storage.project_id)
        self.data_storage.api_client.deployments.delete(self.data_storage.deployment_id)

    def delete_deployments_da(self):
        self.data_storage.api_client.deployments.delete(self.data_storage.deployment_id)
