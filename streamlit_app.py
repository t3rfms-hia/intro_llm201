...
def get_credentials():
    ...
    globals()["api_key"] = os.environ.get('api_key')
    globals()["watsonx_project_id"] = os.environ.get('project_id')
...
def answer_questions():
    ...
    st.markdown(
        ...
        <style>
        .title-container {
            margin-top: 40px;  /* Jarak ke elemen atas */
            margin-bottom: 0px; /* Menghilangkan jarak bawah */
        }
        .date-container {
            margin-top: 0;  /* Jarak kecil antara judul dan tanggal */
        }
        .time-container {
            margin-top: 20px; /* Jarak ke elemen tanggal */
            text-align: right; /* Posisi di sebelah kanan */
    ...
    st.markdown("<div class='title-container'><h3>Tanggal Hari Ini</h3></div>", unsafe_allow_html=True)
    current_date = datetime.now().strftime("%Y-%m-%d")
    st.markdown(f"<div class='date-container'><h4>{current_date}</h4></div>", unsafe_allow_html=True)

    # Placeholder untuk jam yang akan diperbarui setiap detik
    time_placeholder = st.empty()

    # Menambahkan container untuk jam berjalan
    while True:
        current_time = datetime.now().strftime("%H:%M:%S")
 
        time_placeholder.markdown(
            f"<div class='time-container'>Waktu Saat Ini: {current_time}</div>",
            unsafe_allow_html=True
        )
        time.sleep(1)
